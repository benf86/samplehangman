...
Game.prototype.update = function (newData) {
    ...
    if (!!~Object.keys(this.game_data.tries).indexOf(newData.letter)) return this;

    let letterIndex = this.game_data.word.indexOf(newData.letter);

    if (letterIndex > -1) {
        this.game_data.tries[newData.letter] = true;
        this.game_data.word.split('').forEach((e, i) => {
            if (e === newData.letter) this.game_data._word[i] = e;
        });
    } else {
        this.game_data.misses++;
        this.game_data.tries[newData.letter] = false;
    }

    this.data.modified_at = Date();

    if (this.game_data.misses === this.game_data.word.length * 2 ||
        this.game_data.misses === this.game_data.word.length * 2 === 26) {
